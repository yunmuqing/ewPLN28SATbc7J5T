...
$(document).ready(function() {
    ...
    $('.ymq-quantity-change').on('click',function () {
        var num = -1;
        if ($(this).data('type') == 'add'){
            num = 1;
        }
        var value = $('#ymq-quantity-input').val();
        value = Number(value);
        value = value + num;
        if(value < 1){
            value = 1;
        }
        $('#ymq-quantity-input').val(value);
    });
    $('#ymq-quantity-input').on('change',function () {
        var value = $('#ymq-quantity-input').val();
        value = Number(value);
        if(value < 1){
            $('#ymq-quantity-input').val(1);
        }
    });
    ...
    $(".color-save").on('click',function () {
        ...
        if (!$(this).hasClass('disabled')){
            ...
            $.ymqajax({
                url: " /api/branding/quantity.html",
                data: ajaxData,
